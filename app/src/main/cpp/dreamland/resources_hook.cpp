...
android::expected<android::StringPiece16, android::NullOrIOError> (*ResourcesHook::ResStringPool_stringAtS)(
            ...
            case android::ResXMLParser::START_TAG:
                ...
                for (int idx = 0; idx < attrCount; idx++) {
                    ...
                    if (attrNameID >= 0 && (size_t) attrNameID < mTree.mNumResIds &&
                        ...
                        size_t attrNameLen;
                        const char16_t* attrName;
                        if (ResStringPool_stringAt) {
                            attrName = ResStringPool_stringAt(&(mTree.mStrings), attrNameID, &attrNameLen);
                        } else {
                            auto s = ResStringPool_stringAtS(&(mTree.mStrings), attrNameID);
                            attrName = s->data_;
                            attrNameLen = s->length_;
                        }
                        jint attrResID = env->CallStaticIntMethod(XResources, translateAttrId,
                                env->NewString((const jchar*) attrName, attrNameLen), origRes);
...
#define FIND_SYMBOL(symbol, out) handle.GetSymbolAddress((symbol), reinterpret_cast<void**>(&(out)))

#define FIND_SYMBOL_OR_FAIL(symbol, out) \
if (UNLIKELY(!FIND_SYMBOL((symbol), (out)))) { \
...
return false;\
    ...
    if (UNLIKELY(!FIND_SYMBOL(LP_SELECT("_ZNK7android13ResStringPool8stringAtEmPm","_ZNK7android13ResStringPool8stringAtEjPj"), ResStringPool_stringAt))) {
        // Android S
        FIND_SYMBOL_OR_FAIL(LP_SELECT("_ZNK7android13ResStringPool8stringAtEm", "_ZNK7android13ResStringPool8stringAtEj"), ResStringPool_stringAtS);
    }
