...
#include "includes/sandhook_jni.h"
#include <jni.h>
...
Java_com_swift_sandhook_SandHook_setNativeEntry(JNIEnv *env, jclass type, jobject origin, jobject hook, jlong jniTrampoline) {
    ...
    // Dreamland changed: Remove some flags
    constexpr uint32_t kRemoveFlags = 0x0400 /*kAccAbstract*/ | 0x00400000 /*kAccDefault*/ | 0x01000000 /*kAccDefaultConflict*/;
    hookMethod->removeAccessFlags(kRemoveFlags);
...
extern "C" bool register_SandHook(JNIEnv *env, jclass SandHookClass, jclass NeverCallClass) {\
    constexpr int jniMethodSize = sizeof(JNINativeMethod);
    if (env->RegisterNatives(SandHookClass, jniSandHook, sizeof(jniSandHook) / jniMethodSize) < 0) {
        return false;
    }
    if (env->RegisterNatives(NeverCallClass, jniNeverCall, sizeof(jniNeverCall) / jniMethodSize) < 0) {
        return false;
    }
    return true;
}
